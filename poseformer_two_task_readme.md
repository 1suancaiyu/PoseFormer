# add action class task to poseformer but can't work
...
# modified to two-task project
one task is the origin task whick transform 2D to 3D
the task I added is action recognition

## action recognition head
the action lebal as below
```
30 actions
keypoints['positions_2d']['S1']
dict_keys(['Discussion', 'WalkTogether 1', 'Phoning 1', 'Directions 1', 'Sitting 2', 'Posing', 'Purchases 1', 'WalkDog 1', 'Directions', 'Smoking', 'Waiting 1', 'Phoning', 'Eating', 'Waiting', 'Photo 1', 'Walking 1', 'Smoking 1', 'Purchases', 'Discussion 1', 'Photo', 'SittingDown', 'Greeting 1', 'Walking', 'WalkTogether', 'Posing 1', 'WalkDog', 'Greeting', 'Eating 2', 'Sitting 1', 'SittingDown 2'])
...
then I extract the aciton label and add one "head" to the network, loss function used the CrossEntropyLoss() 

## the new network frame as blow

![poseformer_two_task_net_frame](figure\poseformer_two_task_net_frame.png)


start to train
```
python run_poseformer.py -k cpn_ft_h36m_dbb -f 27 -lr 0.00004 -lrd 0.99 -e 1000 --export-training-curves
```

## but get maybe overfit result

![action_class_overfit](figure\action_class_overfit.png)
